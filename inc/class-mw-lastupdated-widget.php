...
class mw_lastupdated_widget extends WP_Widget {
  ...
  public function widget( $args, $instance ) {
    ...
    $post_thumbnail_bool = !empty($instance['post_thumbnail']) ? $instance['post_thumbnail'] : false ;
    ...
    if( $i>0 ) {
      ...
      echo '<ul>';
        ...
        foreach( $significant_updated_posts as $significant_updated_post ) { ?>
          <li class="clear">
            <?php if( $post_thumbnail_bool && has_post_thumbnail( $significant_updated_post->ID ) ) : ?>
              <?php echo get_the_post_thumbnail( $significant_updated_post->ID, array( 100, 100 ) ); ?>
            <?php endif; ?>
        ...
        echo $before_widget;
        _e( 'No posts marked as significantly updated.', 'mw-lastupdated' );
        echo $after_widget;
      }
    }
  }
  ...
  public function form( $instance ) {
    ...
    <p>
      <input class="checkbox" id="<?php echo $this->get_field_id('post_thumbnail'); ?>" name="<?php echo $this->get_field_name('post_thumbnail'); ?>" type="checkbox" <?php checked(isset($instance['post_thumbnail']) ? $instance['post_thumbnail'] : 0); ?> >
      <label for="<?php echo $this->get_field_id( 'post_thumbnail' ); ?>"><?php _e('Should the post thumbnail be showen?','mw-lastupdated'); ?></label>
    </p>
  ...
  public function update( $new_instance, $old_instance ) {
    ...
    /* Strip tags for title and amount to remove HTML (important for text inputs). */
